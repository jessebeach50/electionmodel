...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    button2012.addEventListener('click', handleClick2012);

    const button2012r = document.getElementById('2012 Actual Results');
    button2012r.addEventListener('click', handleClick2012r);
});
...
function handleClick2012() {
    ...
    Papa.parse(csvUrl, {
        ...
        skipEmptyLines: true, // Skip empty lines
        complete: function(results) {
            process2012States(results.data);
            setColorBasedOnChance();
        },
        error: function(error) {
            console.error("Error parsing CSV:", error);
...
//Run the model on the csv imported for 2016
function process2012States(states){
    ...
    states.forEach(s => {
        ...
        var neutral2012ProjectedOnShift = neutral2012 + ((shift1 + shift2) / 2);

        //Compare national polls and state polls to see what the polls think the neutral environment of the state will be in 2024
        var neutral2012ProjectedOnPolls = s.Polls2016 - pollingAverage;

        //Average the above
        var neutral2012Projected = (neutral2012ProjectedOnPolls + neutral2012ProjectedOnShift) / 2
        ...
        //Basic Model using the polling average, account for state poll inaccuracyy by +- 4
        //We'll need to account for polling error in states and nationally to get result so we do a nested for loop
        var maxD = neutral2012ProjectedOnPolls + pollingErrorInMonth + 6;
        var maxR = neutral2012ProjectedOnPolls - pollingErrorInMonth - 6;
        ...
        //Basic Model using the expected shift
        var maxD = neutral2012ProjectedOnShift + 6;
        var maxR = neutral2012ProjectedOnShift - 6;
        ...
        //Wider Variance
        var maxD = neutral2012Projected + 12;
        var maxR = neutral2012Projected - 12;
        ...
        //String that will show when state is hovered over
        infoBoxString = s.State + "\n Actual Election 2012 Results: " + e2012Results + "\nProjected 2012 Result: " + median + "\nDemocrat Win %: " + percentDWin + "\n2012 Polling Average: " + s.Polls2012;
        ...
        let stateData ={
            ...
            Election2012NeutralProjectedShift: neutral2012ProjectedOnShift,
            Election2012NeutralProjectedPolls: neutral2012ProjectedOnPolls,
            Election2012NeutralProjected: neutral2012Projected, 
...
// Shade map by 2012 colors
function handleClick2012r() {
    statesArray.length = 0;
    // Use Papa Parse to fetch and parse the CSV file
    Papa.parse(csvUrl, {
        download: true,
...
//Set the colors to the 2012 result
function setColorsBasedOnResults2012(){
    for(var i = 0; i < statesArray.length; i++) {
        var state2012 = statesArray[i].Election2012Results;
        var stateAbbr = statesArray[i].StateAbbreviation;
        svgState = document.getElementById(stateAbbr);
        if (state2012 > 25){
            try{svgState.style.fill='#040275';}catch{}
        }
        else if(state2012 > 20){
            try{svgState.style.fill='#0300c4';}catch{}
        } 
        else if(state2012 > 15){
            try{svgState.style.fill='#2b28f7';}catch{}
        }    
        else if(state2012 > 10){
            try{svgState.style.fill='#605df5';}catch{}
        } 
        else if(state2012 > 5){
            try{svgState.style.fill='#8a88fc';}catch{}
